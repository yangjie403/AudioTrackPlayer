...
/**
 * 基于 MediaExtractor、MediaCodec 和 AudioTrack 的高级在线音频播放器。
 *
 * 播放链路是：
 * HTTP/HTTPS 音频地址 -> MediaExtractor 读取网络媒体 -> MediaCodec 解码为 PCM ->
 * PcmAudioConverter 统一 PCM 格式 -> AudioTrack 将 PCM 写入系统音频输出。
 *
 * 网络访问、解码和写入全部运行在独立播放线程。播放器对外提供与
 * AdvancedAudioPlayer 对齐的控制能力：播放、暂停、恢复、停止、拖动、循环、进度、
 * 状态、完成和错误回调，以及音频焦点和耳机拔出处理。
 *
 * 服务器最好支持 HTTP Range 请求并提供媒体时长；不支持随机访问的直播流通常无法
 * 可靠执行 seekTo() 或循环播放。
 */
class AdvancedHttpAudioPlayer {
    ...
    companion object {
        private const val TAG = "AdvancedHttpAudioPlayer"
     ...
     * 播放 HTTP/HTTPS 音频。
     *
     * @param audioUrl 网络音频地址。
     * @param loop 是否在输出 EOS 后从头循环。
     * @param headers 传给 MediaExtractor 的 HTTP 请求头，例如鉴权或 User-Agent。
     */
    fun playFromUrl(
        audioUrl: String,
        loop: Boolean = false,
        ...
        require(audioUrl.isNotBlank()) { "Audio URL must not be blank" }
        startInternal(null, loop) { extractor ->
            extractor.setDataSource(audioUrl, headers)
        }
    }
     ...
     * 带 Context 的网络播放版本。
     *
     * 该版本会申请音频焦点并监听耳机/蓝牙设备断开；内部只保存 applicationContext。
     */
    fun playFromUrl(
        context: Context,
        audioUrl: String,
    ...
    private fun startInternal(
        ...
        session.thread = Thread {
            ...
            name = "AdvancedHttpAudioPlayer-${session.id}"
            start()
        }
    }
